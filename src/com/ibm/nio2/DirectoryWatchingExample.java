...
class DirectoryWatchingExample {
    /*public static void main(String[] args) throws Exception {
        ...
        // Use poll() here for 2 seconds because take() will block forever until an event occurs, and we are not expecting one!
        ...
        if ((takeKey != null) && (takeKey == watchKey)) {
            ...
            for (WatchEvent<?> event : takeKey.pollEvents()) {
                System.out.println("An event was found after file modification of kind " + event.kind() + ". The event occurred on file " + event.context() + ".");
            }
        } else {
        ...
        if (takeKey == null) {
            ...
            for (WatchEvent<?> event : takeKey.pollEvents()) {
                System.out.println("An event was found after file deletion of kind " + event.kind() + ". The event occurred on file " + event.context() + ".");
